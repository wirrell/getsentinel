    ...
    for uuid, product in inventory.items():
        ...
        if product['producttype'] == 'S2MSI1C':
            ...
            unprocessed_S2L1C.add(uuid)
                ...
                product['userprocessed'] is False:
            unprocessed_S2L2A.add(uuid)
            pass

        elif product['producttype'] == 'S2MSI2A' and\
                product['userprocessed'] is True:
...
def process(uuid, pipeline, inventory=None):
    ...
    if pipeline == 'S1_WGS84':
        ...
        # move to data dir
        shutil.move(os.path.join(wd, new_fname), os.path.join(DATA_PATH,
                                                              new_fname))
        # remove temp
        shutil.rmtree(wd)
...
def add_S2_processed_to_inventory(uuid, new_file_name, pipeline,
                                  inventory=None):
    """
...
def _make_WGS84_geotiff_inputs(fname):
    ...
    outname1 = '{}_GRDL2_WGS84.tif'.format(os.path.join(DATA_PATH,
                                                        fname.split('.')[0]))
...
def _make_2zone_hdf_inputs(fname, grid_zones):
    ...
    outname1 = '{}_GRDL2_UTM{}.h5'.format(os.path.join(DATA_PATH,
                                                       fname.split('.')[0]),
                                          str(grid_zones[0][0]))
    outname2 = '{}_GRDL2_UTM{}.h5'.format(os.path.join(DATA_PATH,
                                                       fname.split('.')[0]),
                                          str(grid_zones[1][0]))
    t1 = '-Ptarget1="{}"'.format(outname1)
    t2 = '-Ptarget2="{}"'.format(outname2)

    return ([GPT_ROOT_PATH, graph, i1, i2, i3, i4, i5, t1, t2],
            [outname1, outname2])
...
def add_S1_processed_to_inventory(uuid, new_file_name, pipeline,
                                  inventory=None):
    """
    makes a new product entry dict and uses gs_localmanager
    to add to inventory
...
def _get_UTM_zones(product_entry):
    ...
    if product_entry['platformname'] == 'Sentinel-2':
        return [_mgrs_to_zone(product_entry['tileid'])]
